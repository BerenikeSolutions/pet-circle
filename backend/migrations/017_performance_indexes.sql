...
-- Reminders: filtered by pet + status in dashboard
CREATE INDEX IF NOT EXISTS idx_reminders_pet_status
...
-- Conflict flags: pending conflicts for pet
CREATE INDEX IF NOT EXISTS idx_conflict_flags_pet_status
    ON conflict_flags(pet_id, status);
...
-- AI insights: cache lookup by pet + type + expiry
CREATE INDEX IF NOT EXISTS idx_pet_ai_insights_pet_type
    ON pet_ai_insights(pet_id, insight_type, expires_at);
